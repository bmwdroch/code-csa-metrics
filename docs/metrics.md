...
Диапазон: [0, 1] или None (fast mode). 0 = безопасно. Нормализация не требуется.

Реализация: `src/analyzer/metrics.py` (`metric_E1_OSDR`). В текущей версии OSDR = transitive_ratio (доля транзитивных зависимостей). Обогащение данными об активности и сопровождающих не реализовано. Требуется full mode и Maven (`mvnw dependency:list`), в fast mode возвращается None.
...
- Для каждого класса/модуля:
  ...
  - `test_coverage` = покрытие тестами (branch coverage), в текущей версии заглушка = 0.5
  - `duplicate_factor` = LDRI модуля (логические дубли), в текущей версии заглушка = 0.5
  - `abstraction_level` = `1 - concrete_ratio`
- `VFCP = 0.25 * coupling + 0.25 * complexity + 0.20 * (1 - coverage) + 0.15 * duplicates + 0.15 * abstraction`
...
Взвешенное среднее нормализованных значений всех метрик (кроме M1), приведённое к диапазону [0, 1], где 0 = безопасно, 1 = критично. Если метрика недоступна (отсутствует граф, fast mode для E1), она исключается из расчёта, а веса перенормируются на оставшиеся.
...
| E (зависимости) | E1 | 0.06 | Транзитивные зависимости |
