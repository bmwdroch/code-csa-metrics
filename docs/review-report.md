...
```
                    ...
                    ───────────     ────        ───────────     ────
Wall clock          169.7 с         12.3 с      44.3 с          1.5 с
  git clone           1.9 с         1.9 с       0.9 с          0.8 с
  mvnw deps (E1)    157.6 с         —           42.5 с          —
  граф + метрики       9.3 с         9.3 с       0.1 с          0.1 с
Множитель           ×13.8                       ×29.5
```

Иными словами: в fast mode petclinic анализируется за полторы секунды, langchain4j за двенадцать. Полный анализ тех же проектов занимает 170 и 44 секунды соответственно, причём прирост времени обеспечен исключительно скачиванием и разрешением Maven-артефактов для одной метрики из пятнадцати.

При прогретом Maven-кеше (монтирование `out/m2-cache` как `/root/.m2`) petclinic в full mode отрабатывает за 25.8 с вместо 44.3 с: кеш экономит около 42% общего времени, но доминирование E1 сохраняется.
...
```
                    ...
                    full (cold)     fast        full (cold)     fast
                    ───────────     ────        ───────────     ────
CPU пик (%)         348             101         254             ~0
Память пик (МБ)     378             94          200             5
```

Картина аналогична временным замерам. В fast mode langchain4j потребляет одно ядро и 94 МБ (Python tree-sitter удерживает граф из 18360 узлов). В full mode добавляется JVM Maven, поднимающая пик CPU до 3.5 ядер и память до 378 МБ. Для petclinic в fast mode нагрузка настолько мала (5 МБ, 0.08 с на метрики), что Docker stats не успевает зафиксировать пик CPU.
...
──────      ─────       ─────       ─────
47          163         257         0.07 с
2449        18360       589051      8.46 с
```

Экстраполяция: для монорепозитория с 10000 Java-файлов построение графа займёт порядка 35 секунд. Это остаётся приемлемым для этапа формирования релизного кандидата.
...
Выходные файлы:
  langchain4j combined:   2.2 МБ  (раздут из-за M1 topology: 5000 узлов)
  petclinic combined:     92 КБ
```
...
Агрегированный скор разделяет два проекта: spring-petclinic получил 0.273 (умеренный риск), langchain4j -- 0.181 (низкий риск). Соотношение ожидаемо: petclinic является веб-приложением с 17 HTTP-эндпоинтами без авторизации, что создаёт значительную поверхность атаки с прямым доступом к привилегированным операциям (B2=1.0, B1=0.83, A1=0.49). Langchain4j, напротив, не имеет сетевых точек входа, поэтому метрики групп A, B2 и C1 обнуляются. Для библиотеки остаются только структурные характеристики: B1=1.0 (ни одного защитного эшелона на внутренних путях), B4=0.44 (44% catch-блоков не fail-closed), D1=0.5 (три языка в проекте) и повышенный VFCP=0.46 (сложность исправления).
...
Метрика E1 (OSDR) исключена из агрегата в обоих прогонах (fast mode), что приводит к перенормировке весов. В full mode она добавилась бы со значением 0.848 для petclinic, слегка увеличив итоговый скор.
...
```
                    ...
                    ───────────     ────        ───────────     ────
Wall clock          169.7 с         12.3 с      44.3 с          1.5 с
Множитель vs fast   ×13.8           ×1          ×29.5           ×1
```

В fast mode анализ занимает от 1.5 до 12.3 с в зависимости от размера проекта. Для сравнения: типичный запуск SonarQube на проекте аналогичного размера занимает 30-120 с. Прототип в fast mode в десятки раз легче SonarQube и допускает запуск на каждый коммит.

В full mode при холодном кеше анализ занимает от 44 до 170 с, при прогретом кеше от 26 до 100 с. Прирост целиком обусловлен разрешением Maven-зависимостей для одной метрики E1. Запуск full mode на каждый коммит нецелесообразен.
